...
export const getPurchaseRequisitionMaterialDropdown = async(cookie?:string):Promise<PurchaseRequisitionMaterialDropdownType[]>=>{
    try {
        const response = await fetch(`${API_END_POINTS.getMaterialDropdown}`, {
            method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Cookie': cookie as string
