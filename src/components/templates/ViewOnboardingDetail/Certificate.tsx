...
const Certificate = ({ certificateCodeDropdown, ref_no, onboarding_ref_no, OnboardingDetail,isAmendment }: Props) => {
  ...
  return (
    <div className="flex flex-col bg-white rounded-lg p-3 w-full">
      ...
      <div className={`flex justify-end pr-6 pb-6`}><Button className={`bg-blue-400 hover:bg-blue-400 ${isDisabled ? "hidden" : ""}`} onClick={() => { handleAdd() }}>Add</Button></div>
      ...
      <div className="flex justify-end pr-4">
        <Button className={`bg-blue-400 hover:bg-blue-400 ${isDisabled ? "hidden" : ""}`} onClick={() => { handleSubmit() }}>Next</Button>
      </div>
    </div>
  );
};
