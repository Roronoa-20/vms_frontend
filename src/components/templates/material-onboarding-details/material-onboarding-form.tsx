...
const MaterialOnboardingForm: React.FC<MaterialOnboardingFormProps> = (props) => {
  ...
  const isFinalized = approvalStatus === "Sent to SAP" || approvalStatus === "Code Generated by SAP";
  ...
  return (
    <Form {...form}>
      <div className="bg-white min-h-screen p-4 flex flex-col gap-4">
        {/* === SAP MODAL === */}
        <SAPMaterialModal
          isOpen={showSAPModal}
        ...
        {MaterialDetails?.material_request_item?.is_revised_code_new ? (
          <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full flex-1 flex flex-col overflow-hidden">
            ...
            <div className="sticky top-0 z-20 bg-white pb-2">
              <TabsList className="p-1 flex justify-start flex-nowrap bg-slate-100/80 border border-slate-200 shadow-sm rounded-xl gap-2 h-fit text-sm w-full overflow-x-auto scrollbar-hide">
                {tabs.map((tab) => (
                  <TabsTrigger
                    key={tab.id}
                    value={tab.id}
                    disabled={!isFinalized}
                    className="data-[state=active]:bg-[#0C72F5] data-[state=active]:text-white text-black p-2 px-5 rounded-lg text-nowrap disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-semibold"
                  >
                    {tab.label}
                  </TabsTrigger>
                ))}
            ...
            <div className="flex-1 overflow-y-auto px-1 py-1 custom-scrollbar">
              <div className="flex flex-col bg-transparent w-full min-h-fit">
                {tabs.map((tab) => (
