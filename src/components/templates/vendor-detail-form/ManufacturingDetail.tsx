...
const ManufacturingDetail = ({ ref_no, onboarding_ref_no, OnboardingDetail }: Props) => {
  ...
  const handleAddMaterial = () => {
    if (!ManufacturingDetail.material_description || !ManufacturingDetail.hsnsac_code || !ManufacturingDetail.annual_capacity || !manufacturedFile) {
      alert("Please fill in all fields and upload a product image before adding.");
      return;
    }
    const newEntry = {
  ...
  const handleBack = () => {
    router.push(`/vendor-details-form?tabtype=Contact%20Detail&vendor_onboarding=${onboarding_ref_no}&refno=${ref_no}`);
  };
              ...
              <TableBody>
                {materialsTable.map((material, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{material?.material_images?.name ?? "-"}</TableCell>
                    <TableCell>
