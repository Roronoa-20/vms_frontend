...
export default function ASAFormTab() {
  ...
  const handleTabClick = (mainKey: string, subKey?: string) => {
    ...
    router.push(`/asa-form?tabtype=${encodeURIComponent(tab)}`);
  };
    ...
    <div className="p-3 flex flex-col bg-white rounded-xl gap-3">
      {ASAFormTabs.map((tab, i) => {
        ...
        return (
          <div key={i}>
            <div
              className={`flex items-center justify-between cursor-pointer p-2 rounded-lg font-medium transition-all duration-150 ${
                currentTab === tab.key ? 'bg-[#0C72F5] text-white' : 'bg-[#E8F0F7] text-[#0C72F5] hover:bg-[#d1e3f8]'
              }`}
              onClick={() => {
                if (isGovernance) {
