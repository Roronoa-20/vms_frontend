...
const NavbarMenu = ({...Props}:Props) => {
  ...
  return (
    <div ref={outsideClickRef} className={`absolute z-40  bg-white text-black border border-black rounded-lg ease-in-out transition-all flex flex-col p-4 gap-5 ${designation == "Vendor"?"-left-32 -bottom-28":"-left-20 -bottom-20"}`}>
      ...
      <div
        ...
        className={`${designation == "Vendor"?"":"hidden"} text-nowrap flex w-full text-[15px] items-center gap-3 cursor-pointer border-b-2 border-blue-400 hover:bg-slate-100`}
      >
        <svg
          width="19"
          height="19"
        ...
        <h1 className="text-[#2568EF]">View Profile</h1>
      </div>
      <div
        onClick={() => {
          handleLogOut();
