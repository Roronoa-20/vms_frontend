...
const DashboardRejectedQMSTable = ({ dashboardTableData, companyDropdown }: Props) => {
  ...
  const fetchTable = async () => {
    const dashboardRejectedVendorTableDataApi: AxiosResponse = await requestWrapper({
      url: `${API_END_POINTS?.RejectedQMSDocs}?company=${selectedCompany}&vendor_name=${search}&page_no=${currentPage}&page_size=${record_per_page}`,
      method: "GET",
    });
    if (dashboardRejectedVendorTableDataApi?.status == 200) {
      const data = dashboardRejectedVendorTableDataApi?.data?.message;
    ...
    <>
      ...
      <div className="shadow- bg-[#f6f6f7] p-4 rounded-2xl">
        <div className="flex w-full justify-between pb-4">
          <h1 className="text-[20px] text-[#03111F] font-semibold">Rejected QMS Vendors</h1>
            ...
            {table?.length > 0 ? (
              table.map((item, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>
                    <Link href={`/view-onboarding-details?tabtype=Certificate&vendor_onboarding=${item?.name}&refno=${item?.ref_no}`}>
...
export default DashboardRejectedQMSTable;
