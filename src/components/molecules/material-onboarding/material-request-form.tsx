...
import { MaterialRegistrationFormData, Company, Plant, MaterialCategory, MaterialType, UOMMaster } from "@/src/types/MaterialCodeRequestFormTypes";
...
export default function UserMaterialRequestForm({ form, masters, MaterialOnboardingDetails, handleMaterialSearch, searchResults, showSuggestions, handleMaterialSelect, materialSelectedFromList, setMaterialSelectedFromList, setMaterialCodeAutoFetched, setShowSuggestions, materialCodeAutoFetched, setSelectedMaterialType, materialCodeStatus, selectedCodeLogic, setSelectedCodeLogic, latestCodeSuggestions, EmployeeDetails }: UserRequestFormProps) {
  ...
  const employeeCompanies = React.useMemo(() => {
    return EmployeeDetails?.company || [];
  }, [EmployeeDetails]);
  ...
  const isApprovalLoaded = MaterialOnboardingDetails && typeof MaterialOnboardingDetails.approval_status === "string";

  const shouldShowMaterialCode = isCodeGeneratedBySAP || isUseExistingCode || (!isZRND && (selectedMaterialCategory === "R" || selectedMaterialCategory === "P"));
  ...
  useEffect(() => {
    ...
    if (currentValues.material_name_description === details.material_name_description && currentValues.material_company_code === details.material_company_code) { return; }

    if (details.material_company_code) {
      setMaterialCompanyCode(String(details.material_company_code));
    }
    if (details.material_category) {
      form.setValue("material_category", details.material_category);
    }
  ...
  useEffect(() => {
    ...
    const prefillForm = async () => {
      ...
      const targetPlant = plants.find((p) => p.name === details.plant || p.plant_code === details.plant);
      const plantExists = targetPlant ? true : false;

      const typeExists = !details.material_type || materialTypes.some((t: MaterialType) => t.name === details.material_type);
      form.setValue("material_company_code", details.material_company_code || "");
      form.setValue("material_category", details.material_category || "");
      form.setValue("base_unit_of_measure", details.unit_of_measure || "");
      form.setValue("plant_name", plantExists ? targetPlant?.name : "");
      form.setValue("material_type", typeExists ? details.material_type : "");
      form.setValue("material_type_category", typeExists ? details.material_type_category : "");
      form.setValue("material_name_description", details.material_name_description || "");
      form.setValue("material_code_revised", isCodeGeneratedBySAP || isUseExistingCode ? details.material_code_revised || "" : form.getValues("material_code_revised"));
      form.setValue("material_specifications", details.material_specifications || "");
      form.setValue("comment_by_user", details.comment_by_user || "");
      form.setValue("requested_by_name", details.requested_by || name || "");
      form.setValue("requested_by_place", details.requested_by_place || "");

      if (typeExists && details.material_type) setSelectedMaterialType(details.material_type);

      form.trigger(["material_company_code", "plant_name", "material_category", "material_type", "base_unit_of_measure", "material_type_category"]);
  ...
  useEffect(() => {
    if (!MaterialOnboardingDetails) return;
    if (!filteredPlants.length || !filteredMaterialType.length) return;

    const details = MaterialOnboardingDetails;
    const targetPlant = filteredPlants.find((p) => p.name === details.plant || p.plant_code === details.plant);

    if (targetPlant) {
      form.setValue("plant_name", targetPlant.name);
    }

    if (filteredMaterialType.some(t => t.name === details.material_type)) {
      form.setValue("material_type", details.material_type);
      setSelectedMaterialType(details.material_type);
    }
  ...
  useEffect(() => {
    ...
    if (!filteredMaterialType.length) return;
  ...
  const fetchPlantMaster = async (query?: string): Promise<Plant[]> => {
    ...
    let url = baseUrl;
    if (query) { url += `${url.includes('?') ? '&' : '?'}company=${encodeURIComponent(query)}`; }
  ...
  console.log("filteredPlantOptions", filteredPlantOptions);
  ...
  useEffect(() => {
    if (isCodeGeneratedBySAP || isUseExistingCode) return;

    if (!shouldShowMaterialCode && !isCodeGeneratedBySAP && !isUseExistingCode) {
      form.setValue("material_code_revised", "");
  ...
  }, [isCodeGeneratedBySAP, isUseExistingCode, selectedMaterialCategory, selectedMaterialType]);
  ...
  }, [isCodeGeneratedBySAP, isUseExistingCode]);
      ...
      <div className="flex flex-col justify-between bg-white rounded-xl shadow-sm border border-slate-200 p-3 transition-all duration-300 hover:shadow-md">
        <div className="space-y-4">
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Select
                        ...
                        onValueChange={(value) => {
                          field.onChange(value);
                          const selectedCategory = materialCategoryTypeOptions.find((item) => item.material_category_type === value);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-8 mt-4">
            ...
            {isApprovalLoaded && shouldShowMaterialCode && (
                        ...
                        <div className="relative">
                          <FormControl>
                            <Input
                              ...
                              disabled={materialCodeAutoFetched || isCodeGeneratedBySAP || isUseExistingCode}
                            />
                          </FormControl>
                          {!isCodeGeneratedBySAP && !isUseExistingCode && (
                          ...
                          {!isCodeGeneratedBySAP && !isUseExistingCode && latestCodeSuggestions[0] && !materialSelectedFromList && (
                            <p className="mt-1 text-xs text-gray-500">
                              Latest existing code:{" "}
                              <span className="font-semibold">
                                {latestCodeSuggestions[0].material_code || latestCodeSuggestions[0].name || latestCodeSuggestions[0]}
                              </span>
                            </p>
                          )}
                        </div>
