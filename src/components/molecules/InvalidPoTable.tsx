...
type Props = {
  viewInvalidPOTable: DashboardPOTableData["message"];
};
...
const InvalidPoTable = ({ viewInvalidPOTable}: Props) => {
  const [status, setStatus] = useState<"approve" | "reject" | "">("");
  const [date, setDate] = useState("");
  const [comments, setComments] = useState("");
  const [isDialog, setIsDialog] = useState(false);
  const [poNumber, setPONumber] = useState("");
  const [tableData, setTableData] = useState<DashboardPOTableItem[]>(viewInvalidPOTable?.total_po ?? []);
      ...
      <div className="shadow- bg-[#f6f6f7] p-4 rounded-2xl">
        <div className="flex w-full justify-between pb-4">
          <h1 className="text-[20px] text-[#03111F] font-semibold">
            Invalid Purchase and Ongoing Orders
          </h1>
            ...
            {tableData ? (
              tableData?.map((item, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell className="text-center">{item?.vendors_tentative_delivery_date}</TableCell>
                  {/* <TableCell>
                    <Button
                      variant={"outline"}
