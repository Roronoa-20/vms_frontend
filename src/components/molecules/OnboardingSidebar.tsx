...
const OnboardingSidebar = ({ onboarding_refno, refno, isAccountsTeam, vendor_type, nature_of_business }: Props) => {
  ...
  return (
    <div className="p-3 flex flex-col bg-white rounded-xl gap-3 h-fit max-h-[80vh] min-w-fit overflow-y-scroll no-scrollbar ">
      {tabs?.map((item, index) => (
        <div
          // onClick={() => {
          //   router.push(
          ...
          //   );
          // }}
          className={`p-2 ${item.includes(tabType as string) ? "bg-[#0C72F5] text-white" : "bg-[#E8F0F7]  text-[#0C72F5]"} text-nowrap rounded-lg`}
          key={index}
        >
          {item}
        </div>
