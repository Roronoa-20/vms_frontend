...
const DashboardPendingVendorsTable = ({ dashboardTableData, companyDropdown, team }: Props) => {
  ...
  const fetchTable = async () => {
    const normalizedTeam = team === "All" ? "" : team;
    const dashboardPendingVendorTableDataApi: AxiosResponse = await requestWrapper({
      url: `${API_END_POINTS?.PurchaseTeamPendingVendors}?usr=${user}&company=${selectedCompany}&vendor_name=${search}&page_no=${currentPage}&team=${normalizedTeam}`,
      method: "GET",
    });
    console.log("Fetch table API--->", dashboardPendingVendorTableDataApi)
        ...
        <Table>
          <TableHeader className="text-center">
            <TableRow className="bg-[#DDE8FE] text-[#2568EF] text-[14px] hover:bg-[#DDE8FE] text-center">
              ...
              <TableHead className="text-center text-black text-nowrap">Ref No.</TableHead>
