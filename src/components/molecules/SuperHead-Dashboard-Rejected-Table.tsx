...
const DashboardRejectedVendorsTable = ({ dashboardTableData, companyDropdown, team }: Props) => {
  ...
  const fetchTable = async () => {
    ...
    const dashboardRejectedVendorTableDataApi: AxiosResponse = await requestWrapper({
      url: `${API_END_POINTS?.PurchaseTeamRejectedVendors}?usr=${user}&company=${selectedCompany}&vendor_name=${search}&page_no=${currentPage}&team=${normalizedTeam}`,
      method: "GET",
    });
        ...
        <Table>
          <TableHeader className="text-center">
            <TableRow className="bg-[#DDE8FE] text-[#2568EF] text-[14px] hover:bg-[#DDE8FE] text-center">
              ...
              <TableHead className="text-center text-black whitespace-nowrap">Rejected By Designation</TableHead>
