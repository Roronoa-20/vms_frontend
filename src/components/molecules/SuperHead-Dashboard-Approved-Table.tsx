...
const DashboardApprovedVendorsTable = ({ dashboardTableData, companyDropdown, team }: Props) => {
  ...
  const fetchTable = async () => {
    ...
    const dashboardApprovedVendorTableDataApi: AxiosResponse = await requestWrapper({
      url: `${API_END_POINTS?.PurchaseTeamApprovedVendors}?usr=${user}&company=${selectedCompany}&vendor_name=${search}&page_no=${currentPage}&page_size=${record_per_page}&team=${normalizedTeam}`,
      method: "GET",
    });
