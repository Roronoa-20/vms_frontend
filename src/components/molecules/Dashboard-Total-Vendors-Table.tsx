            ...
            {table?.length > 0 ? (
              table.map((item, index) => (
                <TableRow key={index}>
                  ...
                  {!isAccountsUser && (
                    <TableCell><div className={`${(item?.qms_form_filled || item?.sent_qms_form_link) && (item?.company_name == "2000" || item?.company_name == "7000") ? "" : "hidden"}`}><Link href={`/qms-form-details?tabtype=vendor%20information&vendor_onboarding=${item?.name}&company_code=${item?.company_name}`}><Button variant={"outline"}>View</Button></Link></div></TableCell>
                  )}
