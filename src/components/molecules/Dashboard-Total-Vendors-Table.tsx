...
const DashboardTotalVendorsTable = ({ dashboardTableData }: Props) => {
          ...
          {dashboardTableData.total_vendor_onboarding && dashboardTableData.total_vendor_onboarding.length > 0 ? (
            dashboardTableData.total_vendor_onboarding.map((item, index) => (
              <TableRow key={index}>
                ...
                <TableCell>{item?.company_name?item?.company_name:"-"}</TableCell>
                <TableCell>
                  <div
                ...
                <TableCell><Link href={`/view-onboarding-details?tabtype=Company%20Detail&vendor_onboarding=${item?.name}&refno=${item?.name}`}><Button variant={"outline"}>View</Button></Link></TableCell>
