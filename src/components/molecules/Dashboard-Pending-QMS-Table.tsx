...
const DashboardPendingQMSTable = ({ dashboardTableData, companyDropdown }: Props) => {
  ...
  const fetchTable = async () => {
    const dashboardPendingVendorTableDataApi: AxiosResponse = await requestWrapper({
      url: `${API_END_POINTS?.PendingQMDocs}?company=${selectedCompany}&vendor_name=${search}&page_no=${currentPage}&page_size=${record_per_page}`,
      method: "GET",
    });
    if (dashboardPendingVendorTableDataApi?.status == 200) {
      const data = dashboardPendingVendorTableDataApi?.data?.message;
    ...
    <>
      ...
      <div className="shadow- bg-[#f6f6f7] p-4 rounded-2xl">
        <div className="flex w-full justify-between pb-4">
          <h1 className="text-[20px] text-[#03111F] font-semibold">Pending QMS Vendors</h1>
...
export default DashboardPendingQMSTable;
